...
const Pagemodal = (props) => {
    ...
    const submithandler = (event) => {
        ...
        fetch("http://localhost:8000/restaurants", { method: "POST", headers: { authorization: 'Bearer ' + accessToken }, body: formData }).then(res => res.json()).then(resData => console.log(resData)).catch(err => console.log(err))
        props.valid()
    }
    return (
        ...
        <MDBModal open={basicModal} setOpen={setBasicModal} tabIndex='-1' staticBackdrop >
            <MDBModalDialog>
                <MDBModalContent>
                    <MDBModalHeader>
                        <MDBModalTitle>가게 등록</MDBModalTitle>
